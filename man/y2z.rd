...
% Please edit documentation in R/agd.r
\name{y2z}
\alias{y2z}
...
\item{x}{A vector containing the values of the numerical covariate (typically
decimal age or height) at which conversion is desired.  Values are replicated
to match \code{length(y)}.}

\item{sex}{A character vector indicating whether the male (\code{"M"}) of
female (\code{"F"})reference should be used.  Values are replicated to match
\code{length(y)}.}

\item{sub}{A character vector indicating the level of the \code{sub} field of
...
\value{
For \code{y2z()}: A vector with \code{length(y)} elements containing
